...
struct Map {
  private:
    ...
    struct {
        ...
        CallbackContainer<Enemy &, towerdefence::core::Map &, uint32_t>
            on_enemy_release_skill; // uint32_t duration
  ...
  public:
    ...
    // skill
    CallbackHandle on_enemy_release_skill(
        std::function<void(Enemy &, towerdefence::core::Map &map,
                           uint32_t duration)>
            f) {
    ...
    CallbackHandle on_tower_release_skill(
        std::function<void(Tower &, towerdefence::core::Map &map,
                           uint32_t duration)>
            f) {
        CallbackHandle handle{this->assign_id()};
    ...
    CallbackHandle
    on_enemy_move(std::function<void(Enemy &, std::pair<size_t, size_t>,
                                     std::pair<size_t, size_t>)>
                      f) {
        CallbackHandle handle{this->assign_id()};
    ...
    CallbackHandle on_enemy_attacked(std::function<void(Enemy &, Tower &)> f) {
        CallbackHandle handle{this->assign_id()};
...
struct GridRef {
        ...
        for (size_t i = 0; i < map.shape.height_; ++i) {
            for (size_t j = 0; j < map.shape.width_; ++j) {
                if (dis(row, column, i, j) <= radius &&
                    map.grids[map.shape.index_of(i, j)].type !=
                        Grid::Type::BlockOut) {
    ...
    template <class... Args>
        requires std::is_invocable_v<
            typename decltype(map.callbacks_.on_enemy_move)::mapped_type,
            Args...>
    ...
    template <class... Args>
        requires std::is_invocable_v<
            typename decltype(map.callbacks_.on_enemy_attacked)::mapped_type,
            Args...>
    ...
    template <class... Args>
        requires std::is_invocable_v<
            typename decltype(map.callbacks_.on_enemy_death)::mapped_type,
            Args...>
    ...
    template <class... Args>
        requires std::is_invocable_v<
            typename decltype(map.callbacks_
                                  .on_enemy_release_skill)::mapped_type,
            Args...>
    ...
    template <class... Args>
        requires std::is_invocable_v<
            typename decltype(map.callbacks_
                                  .on_tower_release_skill)::mapped_type,
            Args...>
    ...
    template <class... Args>
        requires std::is_invocable_v<
            typename decltype(map.callbacks_.on_escape)::mapped_type, Args...>
    ...
    template <class... Args>
        requires std::is_invocable_v<
            typename decltype(map.callbacks_.on_end)::mapped_type, Args...>
